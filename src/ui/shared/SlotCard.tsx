...
export type SlotCardProps = {
  ...
  focused: boolean;
  onFocusToggle: () => void;
  /** Z1처럼 같은 attribute로만 묶인 리스트에서 꼬리 세그먼트가 중복일 때 제거. */
  hideAttributeSuffix?: boolean;
};
...
export function SlotCard({
  ...
  const baseName = getSlotDisplayName(slotId, ir);
  const displayName = hideAttributeSuffix
    ? baseName.replace(/\.[^.]+$/, '')
    : baseName;
  ...
  return (
    <div>
      <div
        ref={setAnchorEl}
        className={[
          'flex items-center gap-3 px-3 py-2.5 rounded-lg bg-white/80 border transition-all duration-150 cursor-pointer h-[76px]',
          focused && !isMultiMode
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="font-mono text-sm text-stone-900 break-all line-clamp-2">
            {displayName}
          </div>
        ...
        {multiState && (
          <button
            ...
            aria-label={`${component?.states.length ?? 0} ${t('slotCard.states')}`}
          >
            <span className="tabular-nums">{component?.states.length ?? 0}</span>
            <svg
              viewBox="0 0 12 12"
              className="w-3 h-3"
              fill="none"
